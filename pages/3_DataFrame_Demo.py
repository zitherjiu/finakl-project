...
option = st.selectbox('Three aspects',['rating VS age and size','age and size',"skills"])
...
if option =='rating VS age and size':
    ...
    button_click = st.button('remarks')
    if button_click:
...
if option == 'skills':
    ...
    st.write('We also noticed some data on skills. Since our major is information management, we are very interested in this.')
    draw_skill1()
    st.write('From this pie chart, we can recognize that Python has the highest contribution to data jobs, even reaching almost one-third.This number is 10% more than the contribution of Excel, which ranks second.')
    draw_skill2()
    draw_skill3()
    st.write('From the two figures above, we understand the important position of Python in the entire data job. Now we are trying to gain a deeper understanding of the requirements for Python for various types of data jobs.')
    st.write('As shown in this bar chart, data scientists have a huge demand for Python skills, far exceeding that of several other industries.')
    draw_skill4()
    st.write("This pie chart once again proves the importance of Python in today's jobs" )
    img_url ='https://img1.baidu.com/it/u=1429673394,1784562153&fm=253&fmt=auto&app=138&f=JPEG?w=953&h=500'
    st.image(img_url, caption='python')

    
    button_click = st.button('conclusion')
    if button_click:
